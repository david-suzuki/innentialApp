...
import { Button, Form, Cascader, Message } from 'element-react'
import { normalizeSkills } from '../../learning-content/utilities'

export default class MultipleNeededSkillsSelector extends Component {
  constructor(props) {
    super(props)
    this.initialState = {
  ...
  onSkillsChange = (key, value, skill) => {
    if (value.length > 0) {
      ...
      if (key === 0 && this.state.cascaderSelectedSkills.length === 1) {
        this.setState(() => ({
          cascaderSelectedSkills: [
            {
              key: 0,
