...
export default class GenerateCalendarEvents {
  createTestDate() {
    return moment.now()
  }
  ...
  buildUrl(event, type, isCrappyIE /* TODO CHECK FOR IE */) {
    let calendarUrl = ''
    ...
    switch (type) {
      case 'google':
        ...
        if (event.addUsers.length > 0)
          calendarUrl += '&add=' + event.addUsers.join(',')
        break
  ...
  isMobile() {
    ...
    ;(function(a) {
      if (
        /(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows ce|xda|xiino/i.test( //eslint-disable-line
          a
        ) ||
