...
const { CONSTANTS: { UNAUTHORIZED, FORBIDDEN, NOT_FOUND } = {} } = APP
    ...
    graphQLErrors.map(({ message = '', status = 200 }) => {
      if (UNAUTHORIZED === message || status === 401) {
        ...
        if (
          ...
          history.push(
            `/login?to=${history.location.pathname}&query=${encodeURIComponent(
...
export default new ApolloClient({
  ...
  cache: apolloCache,
  addTypename: true
  // connectToDevTools: true // NOTE: THIS KEEPS IT ENABLED IN PROD AS WELL
})
