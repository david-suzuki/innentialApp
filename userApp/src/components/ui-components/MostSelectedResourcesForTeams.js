...
import { fetchMostPopularResourcesForTeams } from '../../api'
import { Statement } from '.'
import { captureFilteredError, LoadingSpinner } from '../general'
import { capitalize } from '../user-onboarding/utilities'

const MostSelectedResources = ({ leaderId }) => {
  const { data, loading, error } = useQuery(fetchMostPopularResourcesForTeams, {
    variables: { leaderId }
  ...
  if (data && data.fetchMostPopularResourcesForTeams) {
    const otherObj = {
      type: 'OTHER',
      users: 0
    }

    const formattedData = data.fetchMostPopularResourcesForTeams.map(item => {
