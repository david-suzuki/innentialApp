...
while test $# -gt 0; do
        case "$1" in
                -h|--help)
                        ...
                        echo "-nc, --no-cache        triggers the deploy forcing building the the nginx docker image from scratch, dismissing the cache)"
...
#echo "$TARGET $SSH_KEY $PASSW $NO_BUILD $HOST_STRING $ENV $VERBOSE $SSL"
bash $(pwd)/deploy/run.sh $TARGET $SSH_KEY $PASSW $NO_BUILD $HOST_STRING $ENV $VERBOSE $SSL $NO_CACHE
